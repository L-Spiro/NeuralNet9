...
namespace nn9 {
	...
	class Stream {
	public :
		Stream( std::vector<uint8_t> &_vStream ) :
			m_vStream( _vStream ),
			m_stPos( 0 ) {
		}
		Stream( const std::vector<uint8_t> &_vStream ) :
		...
		inline bool									ReadStringU16( std::u16string &_u16String ) const {
			...
			_u16String = Utilities::Utf8ToUtf16( reinterpret_cast<const char8_t *>(sTmp.c_str()) );
			return true;
		}
		...
		inline bool									WriteStringU16( const std::u16string &_u16String ) {
			bool bErrored = false;
			std::u8string sTmp = Utilities::Utf16ToUtf8( _u16String, &bErrored );
			if ( bErrored ) {
